...
async def synthesize_elevenlabs(chunk, voice_config=None):
    """
    Synthesize a chunk of text into an AudioSegment using ElevenLabs API.

    Parameters:
    - chunk (str): A single sentence or phrase.
    - voice_config (dict): Dictionary containing voice_id and other voice settings.

    Returns:
    - BytesIO: A buffer containing the generated audio.
    """
    try:
        # Use character-specific voice_id if provided, otherwise use default
...
async def text_to_speech_with_pipelining_elevenlabs(text, voice_config=None):
    """
    ...
    for chunk in chunks:
        if chunk.strip():  # ✅ Ignore empty chunks
            wav_buffer = await synthesize_elevenlabs(chunk.strip(), voice_config)  # ✅ Generate audio
            if wav_buffer:
