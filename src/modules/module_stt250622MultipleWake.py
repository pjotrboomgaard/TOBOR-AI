...
class STTManager:
    ...
    def _detect_wake_word(self) -> bool:
        """
        Detect wake word using multiple PocketSphinx instances running in parallel threads.
        Each instance listens for a specific character name.
        """
        if self.config["STT"].get("use_indicators"):
            self.play_beep(400, 0.1, 44100, 0.6)

        # Get character names from the CHARACTERS config section
        character_names = []
        if 'CHARACTERS' in self.config and self.config['CHARACTERS']:
            character_names = list(self.config['CHARACTERS'].keys())
        
        # Fallback to wake_word if no characters defined
        if not character_names:
            wake_words = [word.strip().lower() for word in self.WAKE_WORD.split(',')]
        else:
            wake_words = character_names
        
        queue_message(f"Listening for: {', '.join(wake_words)}")
        ...
        detected_word = threading.Event()
        detected_character = {'name': None, 'text': None}
        detection_lock = threading.Lock()
        stop_listening = threading.Event()  # Signal to stop all threads

        def listen_for_wake_word(wake_word):
            """Thread function to listen for a specific wake word"""
            try:
                threshold_map = {
                ...
                queue_message(f"DEBUG: Thread started for wake word: '{wake_word}'")
                
                # Create LiveSpeech without audio_timeout parameter
                speech = LiveSpeech(
                    lm=False, 
                    keyphrase=wake_word, 
                ...
                try:
                    for phrase in speech:
                        ...
                        if stop_listening.is_set() or detected_word.is_set():
                            queue_message(f"DEBUG: Thread for '{wake_word}' stopping...")
                            break
                            
                        text = phrase.hypothesis().lower()
                        
                        if text and len(text.strip()) > 0 and wake_word in text:
                            queue_message(f"DEBUG: Thread for '{wake_word}' heard: '{text}'")
                            
                            # Use lock to ensure only one thread sets the result
                            with detection_lock:
                                if not detected_word.is_set():  # Double-check inside the lock
                                    detected_character['name'] = wake_word
                                    detected_character['text'] = text
                                    detected_word.set()  # Signal other threads to stop
                                    stop_listening.set()  # Stop all threads
                                    queue_message(f"DEBUG: Wake word '{wake_word}' detected successfully!")
                                    break
                except Exception as e:
                    queue_message(f"DEBUG: LiveSpeech loop error for '{wake_word}': {e}")
                                    
            except Exception as e:
                queue_message(f"ERROR: Wake word thread for '{wake_word}' failed: {e}")
            finally:
                queue_message(f"DEBUG: Thread for '{wake_word}' finished")

        try:
            # Create and start threads for each wake word
            threads = []
            for wake_word in wake_words:
                thread = threading.Thread(
                    target=listen_for_wake_word, 
                    args=(wake_word,), 
                    name=f"WakeWord-{wake_word}",
                    daemon=True
                )
                threads.append(thread)
                thread.start()
                # Small delay between starting threads to reduce startup load
                time.sleep(0.2)

            queue_message(f"DEBUG: Started {len(threads)} wake word detection threads")

            # Wait for detection or timeout
            timeout_seconds = 30  # Maximum time to listen
            if detected_word.wait(timeout=timeout_seconds):
                # A wake word was detected
                detected_name = detected_character['name']
                detected_text = detected_character['text']
                
                queue_message(f"DEBUG: Detected character: '{detected_name}' from text: '{detected_text}'")
                ...
                queue_message(f"{detected_name.title()}: {wake_response}", stream=True)
                
                if self.wake_word_callback:
                    self.wake_word_callback(detected_text)
                ...
                # Timeout occurred
                queue_message(f"DEBUG: Wake word detection timeout after {timeout_seconds} seconds")
                stop_listening.set()  # Signal threads to stop
                time.sleep(0.5)  # Wait for cleanup
                return False

        except Exception as e:
            queue_message(f"ERROR: Multi-thread wake word detection failed: {e}")
            stop_listening.set()  # Make sure to stop threads on error
            return False
