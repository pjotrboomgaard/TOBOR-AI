...
class MemoryManager:
    ...
    def __init__(self, config, char_name, char_greeting):
        ...
        self.memory_enabled = memory_config.get('enabled', 'True').lower() == 'true'
        self.load_initial_memory_enabled = memory_config.get('load_initial_memory', 'True').lower() == 'true'
        ...
        self.long_mem_use = self.memory_enabled
        self.initial_memory_path =  os.path.abspath(os.path.join(os.path.join("..", "memory", "initial_memory.json")))
        
        if self.memory_enabled:
            queue_message("INFO: Memory system enabled")
            self.init_dynamic_memory()
            if self.load_initial_memory_enabled:
                self.load_initial_memory(self.initial_memory_path)
        else:
            queue_message("INFO: Memory system disabled - no memory will be loaded or saved")
    ...
    def write_longterm_memory(self, user_input: str, bot_response: str):
        ...
        if not self.memory_enabled:
            return  # Skip saving if memory is disabled
    ...
    def write_tool_used(self, toolused: str):
        ...
        """
        if not self.memory_enabled:
            return  # Skip memory writing if disabled
