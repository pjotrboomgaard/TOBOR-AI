...
# -- Chroma settings --
DB_PATH = "/home/pjotska/TARS-AI/src/character/Tobor/database/DB"
COLLECTION_NAME = "my_collection"
SIMILARITY_THRESHOLD = 0.20
TOP_K = 1

openai_key = CONFIG['DB']['embedding_api_key'] or os.getenv("OPENAI_API_KEY_DB")
if not openai_key:
    raise RuntimeError("OPENAI_API_KEY_DB not set in .env")
...
def build_prompt(user_prompt, character_manager, memory_manager, config, debug=False):
    ...
    # DB lookup
    related_fragment_section = ""
    try:
        retrieved = retriever.vectorstore.similarity_search_with_relevance_scores(
            user_prompt, k=TOP_K, score_threshold=SIMILARITY_THRESHOLD
        )
        if retrieved:
            doc, score = retrieved[0]
            emotion = doc.metadata.get("emotion", "een onbekende emotie")
            fragment = doc.page_content.strip().replace("\n", " ")
            related_fragment_section = (
                "### Gerelateerd Fragment (score {:.2f}):\n".format(score) +
                f'Dat doet me denken aan: "{fragment}"\n' +
                f"Het riep bij mij het gevoel op van *{emotion}*.\n---\n\n"
            )
    except Exception as e:
        if debug:
            queue_message(f"DB lookup error: {e}")
    ...
    if related_fragment_section:
        final_prompt = final_prompt.replace(
            "### Response:", related_fragment_section + "### Response:"
        )
