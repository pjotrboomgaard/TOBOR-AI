...
class ZanneAgent(BaseAgent):
    ...
    def load_psychology_profiles(self):
        """Load Zanne's specific psychological patterns"""
        try:
            psychology_path = "character/Zanne/characterpsychology/profiles.json"
            if os.path.exists(psychology_path):
                with open(psychology_path, 'r') as f:
                    self.psychology_data = json.load(f)
        except Exception as e:
            queue_message(f"AGENT: Zanne psychology load failed: {e}")
