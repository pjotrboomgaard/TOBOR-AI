...
Enhanced with name post-processing for personas: Zanne, Pjotr, Mirza, Els
"""
...
class STTManager:
    """
    Manages Speech-to-Text processing for TARS-AI with enhanced name recognition.
    """
    ...
    def enhance_name_recognition(self, text):
        """
        Post-process transcribed text to fix common name misrecognitions.
        Includes phonetic matching for severe Vosk transcription errors.
        
        Args:
            text (str): Original transcribed text
            
        Returns:
            str: Enhanced text with corrected names
        """
        if not text:
            return text
            
        # Define common misrecognitions for Dutch names and personas
        name_fixes = {
            # Zanne variations (including phonetic misrecognitions)
            r'\b(sanne|anne|jan|zaan|zanna|santa|johanna|jana|zane|san|zan)\b': 'zanne',
            
            # Pjotr variations (including severe phonetic errors)
            r'\b(peter|pieter|piet|piotr|pyotr|pietro|pedro|beter|better|beater|peder|petr|computer|heb je er|heb er)\b': 'pjotr',
            
            # Mirza variations
            r'\b(miriam|mira|merza|mirsa|mircea|mirka|miranda|mirza|mersa|mircea)\b': 'mirza',
            
            # Els variations (including phonetic errors)
            r'\b(else|el|ells|elsa|elizabeth|elise|elders|elder|els|ell)\b': 'els',
            
            # Tobor variations (your existing wake word)
            r'\b(robot|tabor|tober|tuber|robert|robo|roboter|heet over|heet|hete over)\b': 'tobor',
            
            # Additional Dutch corrections
            r'\b(wat|wacht|watch)\b': 'wat',
            r'\b(waar|war|were)\b': 'waar',
            r'\b(wie|we|wee)\b': 'wie',
            r'\b(wanneer|when|want)\b': 'wanneer'
        }
        
        # Phase 1: Apply standard fixes
        enhanced_text = text.lower()
        changes_made = []
        
        for pattern, replacement in name_fixes.items():
            if re.search(pattern, enhanced_text, flags=re.IGNORECASE):
                old_text = enhanced_text
                enhanced_text = re.sub(pattern, replacement, enhanced_text, flags=re.IGNORECASE)
                if old_text != enhanced_text:
                    changes_made.append(f"'{old_text.strip()}' → '{enhanced_text.strip()}'")
        
        # Phase 2: Aggressive phonetic matching for severe misrecognitions
        phonetic_fixes = {
            # Catch "beter" -> "pjotr" (severe misrecognition)
            r'\bvertel iets over beter\b': 'vertel iets over pjotr',
            r'\biets over beter\b': 'iets over pjotr', 
            r'\bover beter\b': 'over pjotr',
            r'\bbeter(?=\s|$)': 'pjotr',  # standalone "beter" at word boundary
            
            # Catch "elders" -> "els" (phonetic similarity)
            r'\bvertel iets over elders\b': 'vertel iets over els',
            r'\biets over elders\b': 'iets over els',
            r'\bover elders\b': 'over els',
            r'\belders(?=\s|$)': 'els',  # standalone "elders" at word boundary
            
            # Common phrase patterns that often get misrecognized
            r'\bvertel iets over ([a-z]+er)\b': lambda m: f'vertel iets over {self._guess_name_from_phonetic(m.group(1))}',
        }
        
        for pattern, replacement in phonetic_fixes.items():
            if callable(replacement):
                # Handle lambda functions for dynamic replacement
                enhanced_text = re.sub(pattern, replacement, enhanced_text, flags=re.IGNORECASE)
            else:
                if re.search(pattern, enhanced_text, flags=re.IGNORECASE):
                    old_text = enhanced_text
                    enhanced_text = re.sub(pattern, replacement, enhanced_text, flags=re.IGNORECASE)
                    if old_text != enhanced_text:
                        changes_made.append(f"PHONETIC: '{old_text.strip()}' → '{enhanced_text.strip()}'")
        
        # Log if changes were made
        if enhanced_text != text.lower():
            queue_message(f"STT: Enhanced '{text}' → '{enhanced_text}'")
            if changes_made:
                queue_message(f"STT: Changes applied: {'; '.join(changes_made)}")
        
        return enhanced_text
    
    def _guess_name_from_phonetic(self, word):
        """
        Guess the intended name from phonetically similar words.
        """
        phonetic_map = {
            'beter': 'pjotr',
            'better': 'pjotr', 
            ...
            'elders': 'els',
            'elder': 'els',
            'anders': 'zanne',  # if "anders" sounds like "anne"
            'sander': 'zanne',
            'manner': 'mirza',  # if "manner" sounds like "mira"
            'heet over': 'tobor',
            'hete over': 'tobor',
            'heet': 'tobor',
        }
        
        return phonetic_map.get(word.lower(), word)
    ...
    def _load_vosk_model(self):
        """
        Initialize the Vosk model for local STT transcription with enhanced vocabulary.
        """
        if self.config['STT']['stt_processor'] == 'vosk':
            ...
            queue_message(f"INFO: Vosk model loaded successfully with name enhancement support.")
    ...
    def _load_fasterwhisper_model(self):
        ...
        try:
            ...
            queue_message("INFO: Faster-Whisper model loaded successfully with name enhancement support.")
    ...
    def _load_silero_model(self):
        ...
        try:
            ...
            queue_message("INFO: Silero model loaded successfully with name enhancement support.")
        except Exception as e:
    ...
    def _transcribe_utterance(self):
        """Transcribe the user's utterance using the selected STT processor with name enhancement."""
        try:
    ...
    def _transcribe_with_vosk(self):
        """Transcribe one utterance with the local Vosk model with name enhancement.
        ...
        * Applies name post-processing to fix common misrecognitions.
        """
        recognizer = KaldiRecognizer(self.vosk_model, self.SAMPLE_RATE)
        recognizer.SetWords(True)  # Enable word-level details for better processing
        ...
        original_text = final_result.get("text", "").strip()

        if original_text:
            # Apply name enhancement post-processing
            enhanced_text = self.enhance_name_recognition(original_text)
            
            # Update the result with enhanced text
            final_result["text"] = enhanced_text
            final_result["original_text"] = original_text  # Keep original for reference
    ...
    def _transcribe_with_faster_whisper(self):
        """Transcribe audio using Faster-Whisper with name enhancement."""
    ...
    def _transcribe_silero(self):
        """Transcribe audio using Silero STT with name enhancement."""
        audio_buffer = BytesIO()
        detected_speech = False
        silent_frames = 0
        ...
        original_text = self.decoder(silero_output.cpu())

        # Return transcription result with name enhancement
        if original_text:
            # Apply name enhancement post-processing
            enhanced_text = self.enhance_name_recognition(original_text)
    ...
    def _transcribe_with_server(self):
        """Transcribe audio by sending it to an external server with name enhancement."""
        try:
            audio_buffer = BytesIO()
            silent_frames = 0
            detected_speech = False
            ...
            if response.status_code == 200:
                ...
                if transcription:
                    original_text = transcription[0].get("text", "").strip()
    ...
    def _stt_processing_loop(self):
        ...
        queue_message("INFO: Starting STT processing loop with name enhancement...")
    ...
    def _detect_wake_word(self) -> bool:
        """
        Detect the wake word using enhanced false-positive filtering with name enhancement.
        """
        ...
        try:
            ...
            for phrase in speech:
                ...
                # Apply name enhancement to wake word detection as well
                enhanced_text = self.enhance_name_recognition(text)
                
                if self.WAKE_WORD in enhanced_text or self.WAKE_WORD in text:
                    silent_frames = 0
                    if self.config["STT"].get("use_indicators"):
    ...
    def test_name_enhancement(self):
        """Test the name enhancement function with sample inputs"""
        test_cases = [
            "Hey sanne, how are you?",
            "I need to talk to peter about the project",
            "Where is miriam today?", 
            "Call else please",
            "Robot, can you help me?",
            "Wat doet anne vandaag?",
            "Waar is pieter gebleven?",
            "Kun je mira roepen?",
            "Normal sentence without names",
            "Hey tobor wake up"
        ]
        
        queue_message("INFO: Testing name enhancement:")
        for test in test_cases:
            enhanced = self.enhance_name_recognition(test)
            queue_message(f"Original: '{test}' → Enhanced: '{enhanced}'")

# === Test Function ===
def test_name_enhancement():
    """Standalone test function for name enhancement"""
    print("Testing Enhanced STT Name Recognition")
    print("=" * 50)
    ...
    # Create STT manager instance
    stt = STTManager(mock_config, threading.Event())
    
    # Test cases covering various scenarios
    test_cases = [
        # Dutch name variations
        "Hey sanne, hoe gaat het?",
        "Ik moet peter spreken over het project",
        "Waar is miriam vandaag?",
        "Kun je else roepen?",
        "Robot, kun je me helpen?",
        
        # English variations
        "Call anne for the meeting",
        "Peter is not here today", 
        "Where did mira go?",
        "El needs to see this",
        "The robot is broken",
        
        # Mixed and complex cases
        "Jan en peter moeten samen werken",
        "Anne, mira en els zijn vrienden",
        "Hey tobor, wake up now",
        "Normal sentence without any names",
        "Multiple robots and peters in one sentence"
    ]
    
    print("Testing name enhancement with various inputs:")
    print("-" * 50)
    
    for i, test in enumerate(test_cases, 1):
        enhanced = stt.enhance_name_recognition(test)
        changed = "✓" if enhanced != test.lower() else "○"
        print(f"{i:2d}. {changed} '{test}'")
        if enhanced != test.lower():
            print(f"     → '{enhanced}'")
        print()
    
    print("=" * 50)
    print("✓ = Name enhancement applied")
    print("○ = No changes needed")
    print("\nName enhancement test completed!")

if __name__ == "__main__":
    # Run tests when script is executed directly
    test_name_enhancement()
