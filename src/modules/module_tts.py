...
async def generate_tts_audio(text, ttsoption, voice_config=None, azure_api_key=None, azure_region=None, ttsurl=None, toggle_charvoice=True, tts_voice=None):
    """
    ...
    - voice_config (dict): Dictionary containing voice_id and tts_voice for character-specific voices.
    ...
    """
    try:
        # Use character-specific voice settings if provided
        if voice_config:
            tts_voice = voice_config.get('tts_voice', tts_voice)
            # For ElevenLabs, we'll need to pass the voice_id as well
            
        # Azure TTS generation
        if ttsoption == "azure":
           async for chunk in text_to_speech_with_pipelining_azure(text, voice_config):
                yield chunk
        ...
        # Local TTS generation using local onboard PIPER TTS
            ...
            async for chunk in text_to_speech_with_pipelining_elevenlabs(text, voice_config):
                yield chunk
            ...
            # No TTS - return empty generator for test mode
            return
            yield  # This line never executes but makes it a generator

        else:
            raise ValueError(f"ERROR: Invalid TTS option: {ttsoption}")

    except Exception as e:
        queue_message(f"ERROR: Text-to-speech generation failed: {e}")

async def play_audio_chunks(text, config, voice_config=None):
    """
