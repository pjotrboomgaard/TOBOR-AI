...
class STTManager:
    ...
    def _emit_thinking(self):
        """
        Speak a short filler (e.g. “Hmm…”) the moment VAD decides the user is done,
        so the user hears something while Whisper/Silero/etc. is still transcribing.
        """

        # 1) Respect voice_only mode no matter how TTS config is stored
        tts_cfg = self.config["TTS"]
        if isinstance(tts_cfg, dict):
            voice_only = tts_cfg.get("voice_only", False)
        else:  # dataclass / SimpleNamespace / custom object
            voice_only = getattr(tts_cfg, "voice_only", False)
        if voice_only:
            return  # text will stay hidden if user asked for voice-only

        # 2) Tell ChatUI / external TTS service to start talking *before* queuing the text
        try:
            requests.get("http://127.0.0.1:5012/start_talking", timeout=1)
        except Exception:
            pass

        # 3) Pick a filler and enqueue it
        thinking = random.choice(self.THINKING_RESPONSES)
        character_name = os.path.splitext(
            os.path.basename(self.config["CHAR"]["character_card_path"])
        )[0]
        queue_message(f"{character_name}: {thinking}", stream=True)
