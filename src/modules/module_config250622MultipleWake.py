...
class TTSConfig:
    ...
    def __setitem__(self, key, value):
        """Enable dictionary-like assignment for backward compatibility"""
...
def load_config():
    ...
    # Load CHARACTERS section if it exists
    characters_config = {}
    if 'CHARACTERS' in config:
        characters_config = dict(config['CHARACTERS'])
        queue_message(f"INFO: Loaded {len(characters_config)} character configurations from config.ini")
    else:
        queue_message("WARN: [CHARACTERS] section not found in config.ini")
    ...
    return {
        ...
        "CHARACTERS": characters_config,  # Add the CHARACTERS section
        "LLM": {
        ...
        "DB": {
            "embedding_api_key": os.getenv('OPENAI_API_KEY_DB'),
        },
    }
