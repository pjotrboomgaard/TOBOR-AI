...
def _prepare_request_data(llm_backend, prompt):
    ...
    if llm_backend == "openai":
        ...
        if "reasoning_effort" in CONFIG['LLM']:
            data["reasoning_effort"] = CONFIG['LLM']['reasoning_effort']

    elif llm_backend in ["ooba", "tabby"]:
        url = f"{CONFIG['LLM']['base_url'].rstrip('/')}/v1/completions"
        data = {
            "prompt":     prompt,
            "max_tokens": CONFIG['LLM']['max_tokens'],
            "temperature": CONFIG['LLM']['temperature'],
        ...
        url = f"{CONFIG['LLM']['base_url'].rstrip('/')}/v1/openai/chat/completions"
        data = {
            "model":     CONFIG['LLM']['openai_model'],
            "messages": [
                {"role": "system", "content": CONFIG['LLM']['systemprompt']},
