...
class STTManager:
    ...
    def _transcribe_with_faster_whisper(self):
        ...
        # Get language from config or fallback to English
        language = self.config["STT"].get("whisper_language", "nl")

        segments, _ = self.faster_whisper_model.transcribe(
        audio_data, temperature=0.0, beam_size=1, language=language
